...
export default function BasicModal() {
  ...
  const uploadVideo = async () => {
    try {
      ...
      // Check video file size (Cloudinary free limit is 100MB for videos)
      ...
      data.append("upload_preset", "ls8frk5v");
      // Remove resource_type - let Cloudinary auto-detect
      
      console.log("Video file size:", (video.size / (1024 * 1024)).toFixed(2) + " MB");
      console.log("Video file type:", video.type);

      // Use the same endpoint as images - Cloudinary auto-detects video
      const response = await axios.post(
        "https://api.cloudinary.com/v1_1/dwtsjgcyf/auto/upload",
        data,
        {
          headers: {
      ...
      setVideoLink(response.data.secure_url); // Use secure_url instead of url
      ...
      if (error.code === 'ECONNABORTED') {
        toast.error("Upload timeout - Video toh bahut bari ہے!");
        return null;
      }
      ...
      if (error.response) {
        ...
        if (error.response.status === 400) {
          toast.error("Video format supported nahi ہے");
        } else if (error.response.status === 413) {
          toast.error("Video file bahut bari ہے");
        } else {
          toast.error(`Upload failed: ${error.response.data.error?.message || 'Server error'}`);
        }
      } else if (error.request) {
        console.log("Network error:", error.request);
        toast.error("Internet connection check کریں");
      } else {
